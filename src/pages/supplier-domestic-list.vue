<!--这是供应商列表页面-->
<template>
    <div class="supplier-domestic-list-page" @click="globalFun">
       <div class="list-header">
           <!-- 供应商导航引入 -->
          <supplierHeader :indexPlan="indexPlan" :without="without" @select-type="navIndex" :isHide="isHide" @global="globalFunT" :keyWords="keyWords" @refreshList="domesticInit"  @search="searchListFun(0)"></supplierHeader>
           <div class="list-condition-area">
             <p>您当前所在位置：<a :href="'#/supplier?orgId=' + orgId">首页 > </a><span>国内长线</span></p>
               <ul>
                   <li v-if="false">
       ...
       <div class="list-content">
          <div class="list-search-content">
            <ul>
                <li v-for="(search,index) in searchConditions" :class="searchIndex==index?'active':''" @click.stop.prevent="orderByTypeSearch(search,index)">{{search.name}}<i></i></li>
...
export default {
  ...
  methods: {
    ...
    // 国内长线列表页初始化
    domesticInit() {
      console.log('qqq',this.$route.query.routetypeid);
      // tools.getLocalStorage("routetypeid")==null&&tools.getLocalStorage("routetypeid")==undefined && 
      if(this.$route.query.routetypeid!=undefined){
        // alert('1');
...
//    this.saleIndex = Number(tools.getLocalStorage("saleIndex"));//类型
    ...
    productTagsSearch(obj, index) {
      obj.selected = !obj.selected;
      this.productTagsSelected = [];
      console.log(this.productTagsList,'this.productTagsList');
      for (let i = 0; i < this.productTagsList.length; i++) {
        // if (i != index) {
      ...
      console.log(this.productTagsSelected,'this.productTagsSelected');
      this.searchListFun(0);
    },
    // 销售类型选择
