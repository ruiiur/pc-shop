...
<template>
    <div class="confirm-plan">
        <!-- 供应商导航引入 -->
        <supplierHeader :logo="logoPic" :without="without" @refreshList="confirmPlanInit"></supplierHeader>
        ...
        <div class="plan-content">
            ...
            <div class="con-demand">
                ...
                <div class="plan-state">
                    <span>已确认待支付</span>
                    <div>
                        <p>订单金额：<span>{{comfirmPlan.amt}}</span></p>
                        <div>
                            <a href="javascript:;" @click="cancelDemandOrderFun(comfirmPlan)">取消订单</a>
                            <a href="javascript:;">支付</a>
                        </div>
                    </div>
                </div>
                <h2><img src="../assets/images/plan.png" alt="">定制方案</h2>
                <div class="plan-con">
                    <div>
                        <p>{{comfirmPlan.createddate}}</p>
                        <div class="plan-top">
                            <h2>{{comfirmPlan.subjectname}}</h2>
                            <p>报价<span>{{comfirmPlan.amt}}</span></p>
                        </div>
                        <div class="plan-bottom">
                            <div class="plan-img">
                                <img :src="comfirmPlan.logo" alt="">
                            </div>
                            <div class="plan-info">
                                <h2>{{erpUserDto.deptName}}</h2>
                                <p>定制师<i>{{erpUserDto.userName}}</i><span><i></i>{{erpUserDto.mobile}}</span></p>
                                <div>
                                    <p><label for="">行程天数：</label>{{comfirmPlan.traveldays}}</p>
                                    <p><label for="">出游人数：</label>{{comfirmPlan.adultqty}}成人{{comfirmPlan.childrenqty}}儿童</p>
                                </div>
                                <div class="info-time">
                                    <p><label for="">出发时间：</label>{{comfirmPlan.tourdate}}</p>
                                    <p><label for="">返程时间：</label>{{comfirmPlan.enddate}}</p>
                                </div>
                                <div>
                                    <p><label for="">出发城市：</label>{{comfirmPlan.departure}}</p>
                                    <p><label for="">目的城市：</label>{{comfirmPlan.destination}}</p>
                                </div>
                            </div>
                            <div class="plan-btn">
        ...
        confirmPlanInit(){
            ...
            findPsQuick(params).then(response => {
                ...
                if (response.meta.success == true) {
                    console.log("已确认方案数据获取成功");
                    this.comfirmPlan=response.data;
                    this.cusCusinfo=response.data.cusCusinfo;
                    this.erpUserDto=response.data.erpUserDto;
                } else {
                    console.log("已确认方案数据获取失败");
                }
            })
            .catch(err => {
                console.log("已确认方案数据获取异常", err);
            });
        },
        // 取消定制游订单
