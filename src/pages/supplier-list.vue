<!--这是供应商列表页面-->
<template>
    <div class="supplier-list-page" @click="globalFun">
       <div class="list-header">
           <!-- 供应商导航引入 -->
          <supplierHeader :indexPlan="indexPlan" :without="without" @select-type="navIndex" :isHide="isHide" @global="globalFunT" :keyWords="keyWords" @refreshList="listInit"  @search="searchListFun(0)"></supplierHeader>
           <div class="list-condition-area">
             <p>您当前所在位置：<a :href="'#/supplier?orgId=' + orgId">首页 > </a><span>列表页</span></p>
               <ul>
                   <li v-if="false">
                   ...
                   <li>
                       ...
                       <a href="javascript:;" :class="routeIndex==-1?'active':''" @click.stop.prevent="routeEmpty()">不限</a>
                       <div class="supplier-content">
                           <a href="javascript:;" v-for="(route,index) in routetypes" @click.stop.prevent="routeSearch(route,index)" :class="routeIndex==index?'active':''">{{route.routetype}}</a>
                       </div>
                   </li>
                   <li>
                       <label for="">目的地：</label>
                   ...
                   <li>
                       ...
                       <a href="javascript:;" :class="departureIndex==-1?'active':''" @click.stop.prevent="departureEmpty()">不限</a>
                        ...
                        <p class="departure-city" v-for="(val,key,index) in departureList" v-if="departureIndex==index">
...
export default {
  data() {
    return {
        ...
        departureSelectIndex:-1,//出发地选择值
        without:true,
        //    tripIndex:-1,//旅行天数默认值
        departureIndex: -1, //出发城市默认值
        productIndex: -1, //产品类型默认值
        routeIndex:-1,//线路类型默认值
        //    massIndex:-1,//出团日期默认值
  ...
  methods: {
    ...
    //   this.routetypeid = tools.getLocalStorage("routetypeid");
      // alert(tools.getLocalStorage('orgId'));
      ...
      this.tripType = tools.getLocalStorage("tripType");
      this.saleIndex = Number(tools.getLocalStorage("saleIndex"));
      let tagName = tools.getLocalStorage("tagName");
      if (this.keyWords == null || this.keyWords == undefined) {
        this.keyWords = "";
      }
      
      // console.log(this.supplierIdList);
      let params = {
        siteId: this.siteId,
      ...
      getconditionlist(params)
        .then(response => {
          ...
          if (response.meta.success == true) {
            ...
            //线路类型
            this.routetypes=response.data.routetypes;
            //目的地
            ...
            for(let i=0;i<this.productType.length;i++){
              if(this.tripType==this.productType[i].producttype){
                this.productIndex=i;
                this.proType=this.productType[i].id;
              }
            }
    ...
    routeSearch(obj,index){
        ...
        // 首页目的地查询
        getIndexListDest(params).then(response => {
          console.log(response);
          if (response.meta.success == true) {
            console.log("目的地获取成功");
    ...
    destinationProvinceSelect(obj,index) {
      console.log(obj);
      if(obj.destination.cityName=="国外"){
        this.destination=obj.destination.cityName;
        this.searchListFun(0);
      }
